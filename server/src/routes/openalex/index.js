...
const getData = ({
  countries, endyear, queries, startyear, cursor = '*', previousResponse = [],
}) => {
...
router.route('/openalex')
  .get(async (req, res) => {
    const { countries, endyear, queries, startyear } = req.query;
    const data = await getData({ countries, endyear, queries, startyear });
