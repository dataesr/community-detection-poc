...
const nodeComputeDefaultDegree = (degree, min_degree, max_degree) => {
  ...
  else return (degree / (max_degree - min_degree)) * default_node_range + DEFAULT_NODE_RANGE[0];
};
...
const edgeComputeDefaultWeight = (weight, min_weight, max_weight) => {
  ...
  else return (weight / (max_weight - min_weight)) * default_edge_range + DEFAULT_EDGE_RANGE[0];
};
...
export function dataToGraphology(nodes, edges) {
  ...
  edges.forEach(({ source, target }) =>
    graph.updateUndirectedEdgeWithKey(`(${source}--${target})`, source, target, (attr) => ({
      weight: attr?.weight + 1 || 1,
